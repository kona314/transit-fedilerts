...
export default async function makeRefreshJobs(services: Service[], apex: ActivityPubExpress, client: MongoClient) {
    ...
    return services
        .map(service => new AsyncTask('service-'+service.identifier+'-fetch', async () => {
            console.log('fetching updates')
            const alertsByFeed = await fetchAllAlerts(service)
            return alertsByFeed.flatMap((f, i) => {
                const u = service.gtfsRtAlerts[i]
                const feedUrl = typeof u == "string" ? u : u.url
                return f.map(async (a) => {
                    if (!a.alert) {
                        return Promise.resolve()
                    }
                    const alertInfo = {service: service.identifier, feedUrl, alertId: a.id}
                    const exists = await collection.findOne(alertInfo)
                    if (exists) {
                        return Promise.resolve()
                    }
                    return generateActivityForAlert(a.alert, service.identifier, apex)
                        .then(async (activity) => {
